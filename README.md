...
**For officers**
- **Attendance** — snap a voice channel's members into a logged event, tied to the recurring event schedule; also runnable straight from Discord via `/attendance`, which falls back to the guild's configured attendance voice channel. The officer list filters to the last week, fortnight or 30 days, with the rate sidebar scoped to the same window
