...
**For officers**
- **Attendance** — snap a voice channel's members into a logged event, tied to the recurring event schedule; also runnable straight from Discord via `/attendance`, which falls back to the guild's configured attendance voice channel. Expanding an event gives a table of every member with one of four statuses — *attended*, *no-show (signed up)*, *LOA*, *unexcused* — and the time each row was recorded. Filterable to the last week, fortnight or 30 days, with the rate sidebar scoped to the same window. Each event also stores a frozen copy of the party it fielded, so editing a saved roster later can't rewrite what a past night says it ran with
