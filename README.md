...
Five suites, cheapest first, stopping at the first failure. The first is static and needs no database, so a structural mistake is reported in about a second:
...
**Per-guild config** lives on that guild's row in the `guilds` table — house name, tag, past-name aliases, motto, creed, timezone, guild-night rollover, officer/allow-list/roster role ids, and every channel the bot posts to. Officers edit it themselves at **Guild Settings** in the app; a new tenant is seeded with `node backend/scripts/onboardGuild.js --config guild.json` (`--dry-run` prints what it would write).
