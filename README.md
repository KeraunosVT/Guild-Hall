...
- **Event Calendar** — the week ahead as an agenda: every night in order, every event under it, and **I'm in** next to each one. Every recurring event appears on the nights it falls on, and **signing up for one nobody has opened yet opens it** — the schedule already says the night is happening, so answering it needs no second act of permission. That happens quietly: no Discord post and no ping, because one member tapping a row three weeks out shouldn't make forty phones buzz (officers see the night on the Signups page and Repost it there when they want it announced). After-midnight events are listed under the night they belong to, not the calendar day they start on. Anything you've already filed an LOA for is marked as such, including the awkward case where you're signed up *and* away: both are shown, neither is silently cancelled. It only looks forward — about a month, the same ceiling recurring signups open within — since what already happened is on **My Attendance**
...
- **Late attendance** — a member the snapshot missed can ask to be added, from the website or `/attendance-late`, for 24 hours after attendance was taken. An officer approves or denies it; nobody writes their own attendance. Denials stay on the record, because "did anyone actually look at this" is a question worth being able to answer. Past that window the member is told to ask an officer, and the officer does it directly: on the event page, tick anyone — including the *No Reply* table underneath — and **Mark attended**. That has no deadline, since it is an officer correcting a record they already own rather than approving a claim they can no longer check
...
- **Recurring signups** — a scheduled event can open and post its own signups a set number of days before every occurrence, carrying the capacity, reminder lead and ping role held on the schedule. Nobody has to remember, and no night gets opened twice however many processes are running. Deleting one that has opened is final: a cancelled night stays cancelled rather than reappearing minutes later
...
- **signup semantics** — correctness within one guild rather than isolation between two: capacity, waitlist order, and the races the signup Postgres functions exist to serialise, driven concurrently against a real database. Also the recurring half, where every failure is silent and points the same way — a night that never opens is one nobody signs up for: the horizon, the guild-night rollover for after-midnight events, two sweeps racing, and the ledger that stops a deleted occurrence being resurrected
